...
// import React, { useState, useEffect } from 'react';
// import {
//   Calendar, Clock, Trash2, ChevronLeft, ChevronRight, User, Video
// } from 'lucide-react';
// import './manageschedules.css';
...
//   // Use localStorage or context as needed:
//   const doctorId = localStorage.getItem('doctorId');
...
//     try {
//       setLoading(true);
//       const response = await fetch(`http://localhost:9090/api/appointments/doctorId/${doctorId}`);
//       const data = await response.json();
//       setAppointments(data);
//     } catch (error) {
//       console.error('Error fetching appointments:', error);
...
//   // --- STATUS FIX: treat 'CONFIRMED' and 'BOOKED' as 'booked' ---
//   const getScheduleData = () => {
//     const scheduleData = {};
//     appointments.forEach(appointment => {
//       const appointmentDate = convertToIST(appointment.appointmentDate);
...
//       // Normalize status
//       let normalizedStatus = appointment.status ? appointment.status.toLowerCase() : 'available';
...
//     // Add available slots (user-defined)
//     Object.keys(availableSlots).forEach(dateKey => {
//       if (!scheduleData[dateKey]) scheduleData[dateKey] = [];
//       scheduleData[dateKey] = [...scheduleData[dateKey], ...availableSlots[dateKey]];
//     });
...
//       endTime: newSlot.endTime,
//       type:
//         newSlot.consultationType === 'video'
//           ? 'Video Consultation'
//           : newSlot.consultationType === 'inperson'
//           ? 'In-person'
//           : 'Both',
//       patient: null,
//       status: 'available'
...
//                 style={{ minHeight: '300px', border: isToday ? '2px solid #10b981' : '2px solid #F3F4F6' }}>
//                 <div style={{ textAlign: 'center', marginBottom: 15 }}>
//                   <h3 style={{ margin: 0, fontSize: 14, color: '#6b7280' }}>{weekDays[index]}</h3>
//                   <h2 style={{ margin: '5px 0', fontSize: 18, color: '#1a1a1a' }}>{date.getDate()}</h2>
//                   {isToday && <span style={{ fontSize: 12, color: '#10b981', fontWeight: 600 }}>Today</span>}
//                 </div>
//                 <div style={{ marginBottom: 15 }}>
//                   {daySchedule.length === 0 ? (
//                     <p style={{ fontSize: 12, color: '#6b7280', textAlign: 'center', margin: '20px 0' }}>
//                       No appointments
//                     </p>
//                   ) : (
//                     daySchedule.map(slot => (
...
//                         border: `1px solid ${slot.status === 'booked' ? '#DBEAFE' : '#DCFCE7'}`
//                       }}>
//                         <div style={{ fontWeight: '600', marginBottom: 2 }}>{slot.startTime} - {slot.endTime}</div>
//                         <div style={{ color: '#6b7280', marginBottom: 2 }}>{slot.type}</div>
//                         {slot.patient && <div style={{ color: '#059669', fontWeight: 500 }}>{slot.patient}</div>}
//                         <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
//                           {slot.status === 'available' && (
//                             <button onClick={() => handleDeleteSlot(dateKey, slot.id)} style={{
//                               background: 'none',
//                               border: 'none',
//                               cursor: 'pointer',
//                               padding: 2,
...
//     const scheduleData = getScheduleData();
//     const currentWeekDates = getWeekDates(currentDate);
//     let weeklyAvailable = 0, weeklyBooked = 0, totalHours = 0, videoConsultations = 0;
//     currentWeekDates.forEach(date => {
//       const dateKey = formatDate(date);
//       const daySchedule = scheduleData[dateKey] || [];
//       daySchedule.forEach(slot => {
//         if (slot.status === 'available') weeklyAvailable++;
//         else if (slot.status === 'booked') {
//           weeklyBooked++;
//           if (slot.type && slot.type.toLowerCase().includes('video')) videoConsultations++;
//         }
//         totalHours += 1;
//       });
//     });
//     return { weeklyAvailable, weeklyBooked, totalHours, videoConsultations };
//   };
...
//       </div>
//       {/* Quick Stats */}
//       <div className="doctors-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', marginBottom: 30 }}>
//         <div className="card" style={{ textAlign: 'center' }}>
//           <Calendar className="w-8 h-8" style={{ color: '#10b981', margin: '0 auto 10px' }} />
//           <h3 style={{ margin: '0 0 5px 0', fontSize: '1.5rem', color: '#1a1a1a' }}>{stats.weeklyAvailable}</h3>
//           <p style={{ margin: 0, color: '#6b7280' }}>Available Slots This Week</p>
...
//   // Updated calculateStats to accept view parameter to compute weekly or monthly stats dynamically
//   const calculateStats = (view) => {
//     const scheduleData = getScheduleData();
//     let datesToCheck = [];

//     if (view === 'weekly') {
//       datesToCheck = getWeekDates(currentDate);
...
//         datesToCheck.push(new Date(year, month, day));
//       }
//     }
...
//   const stats = calculateStats(activeView);
...
//       const dateKey = formatDate(appointmentDate);
//       const startTime = formatTime(appointment.appointmentDate);
//       // Fix: Use 30 minutes duration (instead of 60)
//       const endTime = formatTime(new Date(new Date(appointment.appointmentDate).getTime() + 30 * 60 * 1000));
