...
const ProfileComponent = () => {
  ...
  const fetchPatientData = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(`http://localhost:9090/api/patients/user/${customUserId}`);
      if (!response.ok) {
  ...
  const updatePatientData = async (updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:9090/api/patients/${profile.id}/update`, {
        method: 'POST',
